...
class AtramRosterLoad(object):
	...
	def process(self):
		try:
			...
			fileData = self.getImportFileBySite("roster.txt")
			if self.destroy == '1':
				self.doDestroy()
			for row in fileData:
	...
	def resolveKeys(self,rowDict):
		rowDict['title_id'] = self.getTableKey('wf_title',rowDict.get('titlecode'))

		if not rowDict.get('title_id',None):
			print rowDict.get('titlecode')
			sys.exit(0)
	...
	def getRowDict(self,row):
		rowDict = {}
		rowDict['community'] = row[0].strip()
		rowDict['username'] = row[1].strip()
		rowDict['first_name'] = row[2].strip()
		rowDict['last_name'] = row[3].strip()
		rowDict['middle_name'] = row[4].strip()
		rowDict['suffix'] = row[5].strip()
		rowDict['email'] = row[6].strip()
		rowDict['employee_nbr'] = row[7].strip()
		rowDict['deptcode'] = row[8].strip()
		rowDict['pcn'] = row[9].strip()
		rowDict['is_primary'] = self.resolveBoolean(row[10].strip())
		rowDict['titlecode'] = row[11].strip()
		rowDict['start_date'] = self.resolveDate(row[12].strip())
		if len(rowDict) > 13:
			rowDict['end_date'] = self.resolveDate(row[13].strip())
		return rowDict
...
class DataLoadInterface:
	...
	def get_parser(self):
		...
		parser.add_option('-d', '--dbname', dest='dbname', default='demo', help='database name (default=mpsdev')
