	...
	def getContent(self):
		...
		try:
			...
			if evaluatorKey:
				...
				evalContainer = self.workflow.getContainer('education_eval')
				if evalContainer:
			...
			context['lastNameFirst'] = stringUtils.constructLastCommaFirstName(person.get('first_name',''),person.get('last_name',''),person.get('middle_name',''),person.get('suffix',''))
			context['firstNameLast'] = stringUtils.constructFullName(person.get('first_name',''),person.get('last_name',''),person.get('middle_name',''),person.get('suffix',''))
			...
			context['certificationHeader'] = 'CERTIFICATION OF MEDICAL EDUCATION FOR MEDICAL SCHOOL GRADUATES'
			context['deanRegistrarHeader'] = 'THIS SECTION TO BE COMPLETED BY THE DEAN OR REGISTRAR OF MEDICAL SCHOOL'
			context['schoolType'] = 'medical school'
			if evaluatorDict.get('program','') == 'OptometrySchool':
				context['certificationHeader'] = 'CERTIFICATION OF EDUCATION COMPLETION OF DOCTORATE DEGREE FOR OPTOMETRISTS'
				context['deanRegistrarHeader'] = 'THIS SECTION TO BE COMPLETED BY THE DEAN OR REGISTRAR OF SCHOOL WHERE EDUCATION WAS COMPLETED'
				context['schoolType'] = 'school'
			elif evaluatorDict.get('program','') == 'DentalSchool':
				context['certificationHeader'] = 'CERTIFICATION OF DENTAL EDUCATION FOR DENTAL SCHOOL GRADUATES'
				context['deanRegistrarHeader'] = 'THIS SECTION TO BE COMPLETED BY THE DEAN OR REGISTRAR OF DENTAL SCHOOL'
				context['schoolType'] = 'dental school'
			elif evaluatorDict.get('program','') == 'DoctoralProgramPhDPsyD':
				context['certificationHeader'] = 'CERTIFICATION OF EDUCATION COMPLETION OF DOCTORATE DEGREE FOR PSYCHOLOGISTS'
				context['deanRegistrarHeader'] = 'THIS SECTION TO BE COMPLETED BY THE DEAN OR REGISTRAR OF SCHOOL WHERE EDUCATION WAS COMPLETED'
				context['schoolType'] = 'school'

			context['admissionDate'] = ''
			if evaluatorDict.get('admission_date',''):
				context['admissionDate'] = datetime.datetime.strptime(evaluatorDict.get('admission_date',''),'%Y-%m-%d').strftime('%b %d, %Y')
